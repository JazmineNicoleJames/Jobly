  ...
  try {
    ...
    if(Object.keys(req.query).length !== 0){
      ...
      if(minEmployees > maxEmployees){
        throw new BadRequestError(`minimum employees can not be greater than maximum.`, 400)
      }

      const companies = await Company.filter(name, minEmployees, maxEmployees);

      if(companies.length === 0){
        return res.status(404).json({message: "No companies matching that filter"})
      }
      return res.json({ companies })
    } else {
      /*  find all companies without a filter*/
